...
Builds the [SYSTEM]/[MANDATE]/[LEDGER]/[QUESTION] prompt (guideline.md §7's
instruction hierarchy; [RETRIEVED] is always omitted -- no RAG until M5),
calls the shared `get_structured_completion` primitive against a schema that
deliberately excludes `verification` (the LLM never self-grades
verification), and applies the one confidence-calibration rule M3 owns.
"""
